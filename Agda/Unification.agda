...
open import Data.Fin as Fin using (Fin; suc; zero) renaming (raise to fraise)
...
open import Data.List as List using (List; _∷_; []; monad; map)
...
module Unification
  ...
  lam-inj : ∀{n t1 t2} → lam {n} t1 ≡ lam {n} t2 → t1 ≡ t2
  lam-inj refl = refl

  lvar-inj : ∀{n m₁ m₂} → lvar {n} m₁ ≡ lvar {n} m₂ → m₁ ≡ m₂
  lvar-inj refl = refl
  ...
  mutual
    ...
    _≟-Term_ (lam _) (lit _)     = no (λ ())
    _≟-Term_ (lam _) (var _)     = no (λ ())
    _≟-Term_ (lam _) (con _ _)   = no (λ ())
    _≟-Term_ (lam _) (lvar _)    = no (λ ())
    _≟-Term_ (lit _) (lam _)     = no (λ ())
    _≟-Term_ (var _) (lam _)     = no (λ ())
    _≟-Term_ (con _ _) (lam _)   = no (λ ())
    _≟-Term_ (lit _) (lvar _)    = no (λ ())
    _≟-Term_ (var _) (lvar _)    = no (λ ())
    _≟-Term_ (con _ _) (lvar _)  = no (λ ())
    _≟-Term_ (lvar _) (lit _)    = no (λ ())
    _≟-Term_ (lvar _) (con _ _)  = no (λ ())
    _≟-Term_ (lvar _) (lam _)    = no (λ ())
    _≟-Term_ (lvar _) (var _)    = no (λ ())
    ...
    _≟-Term_ {n} (lam t1) (lam t2) with t1 ≟-Term t2
    ...| yes t1=t2 = yes (cong lam t1=t2)
    ...| no  t1≠t2 = no (t1≠t2 ∘ lam-inj)
    _≟-Term_ (lvar i) (lvar j) with i ≟-ℕ j
    ...| yes i=j = yes (cong lvar i=j)
    ...| no  i≠j = no (i≠j ∘ lvar-inj)
  ...
  replace f (var i) = f i
  replace f (lam t) = lam (replace f t)
  replace f (lvar i) = lvar i
  ...
  check x₁ (lam t)    = lam <$> check x₁ t
  check x₁ (lvar _)   = nothing
  ...
  mutual
    ...
    unifyAcc (con _ _) (lam _)  (n , nil) = nothing
    unifyAcc (con _ _) (lvar _) (n , nil) = nothing
    unifyAcc (lit _) (lam _)    (n , nil) = nothing
    unifyAcc (lit _) (lvar _)   (n , nil) = nothing
    unifyAcc (lvar _) (lam _)   (n , nil) = nothing
    unifyAcc (lvar _) (lit _)   (n , nil) = nothing
    unifyAcc (lvar _) (con _ _) (n , nil) = nothing
    unifyAcc (lvar _) (var _)   (n , nil) = nothing
    unifyAcc (lvar j) (lvar i) p with i ≟-ℕ j
    ...| yes i=j = just p
    ...| no  i≠j = nothing
    unifyAcc (lam t₁) (lam t₂) p = unifyAcc t₁ t₂ p
    unifyAcc (lam _)  _        _ = nothing
