...
is a relator if and only if it preserves converses, theorem 5.1 in \cite{Bird97}.
...
to be impossible\footnote{%
%%%% BEGIN FOOTNOTE
With the help of Coinduction one can define fixed points, but this triggers loads of other
problems when using such fixed points.
%%%% END FOOTNOTE
...
that all polynomial functors can be encoded in that fashion, where $S$ represents
...
as a constructor of arity $P\; s$ \cite{lof84}.
...
which gives out the generic induction principle for W-types. The important question then becmoes: are we 
talking about \emph{the same} lists? And the answer is yes. Abbot et al. did a lot of work on container types
...
\begin{TODO}
  \item explain catas --- relational
  \item give a small intro to W-types.
  \item present our own "w-rec", W-cata-F scheme.
  \item Relator typeclass
\end{TODO}
